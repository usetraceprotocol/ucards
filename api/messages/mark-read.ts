 ...
 *
 * Marks all messages from a specific sender as read for the authenticated user.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!sender_username || typeof sender_username !== 'string') {
      return res.status(400).json({ error: 'sender_username is required' });
    }

    const cleanSender = sender_username.replace(/^@/, '').trim();
